...
check_inputs() {
  ...
  if [ -n "$ACT" ] &&
    ...
    MESSAGE='Warning - Unsupported configuration: post_comment is not compatible with nektos/act. (unsupported-repo-notation)' \
    check_yaml_key_value "$workflow_path"
  fi
  if [ -n "$ACT" ] &&
...
get_hunspell_stem() {
  echo "$1" | perl -pe 's{/src/hunspell/index.*}{};s{.*/}{}'
}
  ...
  if [ -n "$extra_dictionaries" ]; then
    ...
    (
      ...
      if [ "$dictionary_base" = index.dic ]; then
        dictionary_base="$(get_hunspell_stem "$dictionary_base")".dic
      fi
